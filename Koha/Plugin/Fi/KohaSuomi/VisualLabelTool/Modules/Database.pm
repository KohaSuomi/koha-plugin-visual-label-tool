...
sub setFieldData {
    ...
    my $sth=$self->dbh->prepare("INSERT INTO ".$self->fields." (label_id,name,type,top,`left`,`right`,`bottom`,fontsize, fontfamily, fontweight, whitespace, height, overflow) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
    return $sth->execute(@params);
...
sub updateFieldData {
    ...
    label_id = ?, name = ?,type = ?, top = ?, `left` = ?,`right` = ?,`bottom` = ?,fontsize = ?, fontfamily = ?, fontweight = ?, whitespace = ?, height = ?, overflow = ?
    WHERE id = ?;");
    return $sth->execute(@params);
